...
CRITICAL: Return ONLY the final raw prompt string. Do not wrap it in quotes, do not include any introductory or concluding text (such as "Here is your prompt:"), and do not explain your choice. Just return the prompt text itself.`,
  ...
  console.log('\n🎨 Generating cover image with OpenAI (gpt-image-1.5)...');
  let fileBuffer;
  try {
    const response = await fetch('https://api.openai.com/v1/images/generations', {
      ...
      body: JSON.stringify({
        ...
        prompt: promptText,
        n: 1,
        size: '1024x1024' // Using landscape format
      })
    });

    let data = await response.json();
    if (!response.ok) {
      // If 1536x1024 fails due to size, fallback to 1024x1024
      if (JSON.stringify(data).includes('size')) {
        console.log('⚠️ 1536x1024 size not supported. Falling back to 1024x1024...');
        ...
        if (!retryRes.ok) throw new Error(data.error ? data.error.message : JSON.stringify(data));
      } else {
    ...
    if (data.data && data.data[0] && data.data[0].b64_json) {
      fileBuffer = Buffer.from(data.data[0].b64_json, 'base64');
    ...
    console.error('❌ Failed to generate image with OpenAI:', err.message);
    process.exit(1);
  }
