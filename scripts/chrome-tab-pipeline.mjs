...
const SKIP_URL_PATTERNS = [
  ...
  'goibibo.com', 'icicibank.com', 'icici.bank.in', 'fal.ai', 'publication.media', 'ollama.com'
];
...
async function rewriteWithOllama(rawText, url, articleId, relatedLinks, suggestedCategory = null, suggestedAuthor = 'Preeti Gunjan') {
  const systemPrompt = `You are a senior travel journalist writing for traveldailypost.com — a blog covering travel news, airline updates, destination guides, hotel reviews, and cruise news.
${suggestedCategory ? `- You MUST set the category in the frontmatter to EXACTLY "${suggestedCategory}".` : ''}
...
- CRITICAL: Under "## Related Travel Guides", you MUST copy the EXACT markdown links provided above — do NOT invent, change, or add any links. Use them exactly as-is, word for word.`;
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      ...
      { role: 'user', content: "Rewrite this article for traveldailypost.com:\n\nSource URL: " + url + "\n\nContent:\n" + rawText.slice(0, 9000) }
    ],
    temperature: 0.3,
    max_tokens: 4096,
  });

  let md = response.choices[0].message.content.trim();
  if (md.startsWith('```')) md = md.replace(/^```[a-z]*\n?/, '').replace(/```$/, '').trim();
  return md;
}
...
function saveArticle(md, articleId, forcedCategory = null, forcedAuthor = null, slugPool = []) {
  ...
  md = md.replace(/^category:\s*.*$/m, `category: "${category}"`);
  ...
  md = md.replace(/^slug:\s*.*$/m, `slug: "${slug}"`);

  // Fix coverImage placeholder path
  md = md.replace(/^coverImage:\s*.*$/m, `coverImage: "https://images.traveldailypost.com/articles/${category}/${YEAR}/${MONTH}/${slug}.jpg"`);

  // Ensure ID is correct
  md = md.replace(/^id:\s*.*$/m, `id: ${articleId}`);

  // Ensure date and updatedDate are set to the correct formatted date
  md = md.replace(/^date:\s*.*$/m, `date: "${TODAY}"`);
  md = md.replace(/^updatedDate:\s*.*$/m, `updatedDate: "${TODAY}"`);
...
async function main() {
  ...
  for (const url of articleUrls) {
    ...
    try {
      ...
      // Rewrite with Claude (fallback to Ollama if needed)
      const relatedLinks = pickRelatedLinks(slugPool);
      let md;
      try {
        console.log('   ↳ Rewriting with Claude...');
        md = await rewriteWithClaude(rawText, url, nextId, relatedLinks, sourceCategory, author);
      } catch (claudeErr) {
        console.warn(`   ⚠️ Claude rewrite failed: \${claudeErr.message}. Falling back to local Ollama model...`);
        md = await rewriteWithOllama(rawText, url, nextId, relatedLinks, sourceCategory, author);
      }
